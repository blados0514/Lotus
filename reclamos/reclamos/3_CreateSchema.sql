...
CREATE TABLE process.zdp_lotus_reclamos_cat_tiempo AS
WITH fechas AS (
...
SELECT
  ...
  concat(dtFechaCreacion.value, ' - ', rdClienteRazon.value, ' - ', numAbonoTemporal.value, ' - ', txtTipoProducto.value, ' - ', txtTipoReclamo.value ) AS busqueda
FROM 
...
SELECT 
  ...
  txtIdSubreclamo.value AS txtIdSubreclamo,
...
FROM
  Process.Zdp_Lotus_Reclamos_Reclamos_TablaIntermedia
  LATERAL VIEW posexplode(numFilaTransac.value) Fila AS posicion, Fila
  LATERAL VIEW posexplode(numValoresTransac.value) Valor  AS posicion2, Valor
  LATERAL VIEW posexplode(dtFechasTransac.value) Fecha AS posicion3, Fecha
  LATERAL VIEW posexplode(txtOficinasTransac.value) Oficina AS posicion4, Oficina
  LATERAL VIEW posexplode(txtReferenciasTransac.value) Referencia AS posicion5, Referencia
  LATERAL VIEW posexplode(txtMonedasTransac.value) Moneda AS posicion6, Moneda
  LATERAL VIEW posexplode(txtCiudadesTransac.value) Ciudad AS posicion7, Ciudad
  LATERAL VIEW posexplode(txtCuentasTransac.value) NoCuenta AS posicion8, NoCuenta
  LATERAL VIEW posexplode(txtChequesTransac.value) NoCheque AS posicion9, NoCheque
  LATERAL VIEW posexplode(dtConsignacionesTransac.value) FechaConsignacion AS posicion10, FechaConsignacion
WHERE
  posicion = posicion2
  AND posicion = posicion3
  AND posicion = posicion4
  ...
  AND posicion = posicion9
  AND posicion = posicion10;
...
SELECT
  ...
  W.posicion,
  W.e_attachments,
  CONCAT(X.Valor1,Y.Valor1,'/',W.UniversalID,'/',W.e_attachments,Z.Valor1) link
FROM
  (
    SELECT
  UniversalID,
...
FROM
  Process.Zdp_Lotus_Reclamos_Reclamos_TablaIntermedia
  lateral view posexplode(attachments) e_attachments AS posicion, e_attachments
  ) W
  ...
  CROSS JOIN Default.parametros Z ON (Z.CodParametro="PARAM_LOTUS_CAYMAN_TOKEN")